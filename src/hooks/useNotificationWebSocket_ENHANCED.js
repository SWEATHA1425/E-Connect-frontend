...
export const useNotificationWebSocket = () => {
  ...
  const apiBaseUrl = import.meta.env.VITE_HOST_IP || "https://e-connect-backend-2l74.onrender.com";
        ...
        try {
          ...
          if (data.type === 'notification') {
            ...
            if (data.data && data.data.userid === userid) {
              ...
              // 🚀 ENHANCED: Show both desktop and in-app notifications
              if (data.data) {
                const notificationData = {
                  ...
                  onClick: () => {
                    ...
                    window.focus();
                    // Emit a custom event to navigate
                ...
                // Show desktop notification (works outside E-Connect)
                desktopNotificationManager.show(notificationData);

                // Also emit event for in-app toast notification
                window.dispatchEvent(new CustomEvent('econnect-new-notification', {
                  detail: notificationData
                }));
                
                console.log('🔔 Global notification shown:', data.data.title);
              }
            } else {
  ...
  // Request notification permission using the enhanced manager
  const requestNotificationPermission = useCallback(async () => {
    return await desktopNotificationManager.requestPermission();
  }, []);
