...
export default function HRDocsReview() {
  ...
  const res = await axios.get("https://e-connect-backend-2l74.onrender.com/get_all_users");
  ...
  const fetchAssignedDocs = async (userId) => {
    try {
  const res = await axios.get(`https://e-connect-backend-2l74.onrender.com/documents/assigned/${userId}`);
  ...
  await axios.post("https://e-connect-backend-2l74.onrender.com/assign_docs", {
  ...
  const handleVerify = async (userId, docName) => {
    try {
  await axios.put("https://e-connect-backend-2l74.onrender.com/review_document", {
        userId,
        docName,
        status: "verified",
  ...
  await axios.delete("https://e-connect-backend-2l74.onrender.com/assigned_doc_delete", {
        data: { userId, docName },
      });
      setAssignedDocs((prev) => ({
        ...prev,
    ...
    <span className="flex items-center gap-1 px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded-full">
      <Clock size={12}/> Pending
                    ...
                    <td className="py-2 px-4 flex gap-2">
                      {doc.fileUrl && (
                        <a
                          href={`https://e-connect-backend-2l74.onrender.com/${doc.fileUrl}`}
                          target="_blank"
                          rel="noreferrer"
