...
export default function Chat() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
  const res = await fetch("https://e-connect-backend-2l74.onrender.com/get_all_users");
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
  const res = await fetch(`https://e-connect-backend-2l74.onrender.com/get_user_groups/${userid}`);
        const data = await res.json();
        setGroups(data);
      } catch (err) {
  ...
  const openWebSocket = (chatType = "user", chatId = "") => {
    ...
    const url =
      chatType === "group"
  ? `wss://e-connect-backend-2l74.onrender.com/ws/group/${chatId}`
  : `wss://e-connect-backend-2l74.onrender.com/ws/${userid}`;
  ...
  const handleContactClick = async (contact) => {
    try {
  const res = await fetch(`https://e-connect-backend-2l74.onrender.com/get_EmployeeId/${encodeURIComponent(contact.name)}`);
      const data = await res.json();
  ...
  const historyRes = await fetch(`https://e-connect-backend-2l74.onrender.com/history/${chatId}`);
      if (historyRes.ok) {
  ...
  const handleGroupClick = async (group) => {
    ...
    try {
  const res = await fetch(`https://e-connect-backend-2l74.onrender.com/group_history/${group._id}`);
      if (res.ok) {
  ...
  const handleRemoveGroup = async (group) => {
    ...
    try {
  const res = await fetch(`https://e-connect-backend-2l74.onrender.com/delete_group/${group._id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const sendThreadMessage = async () => {
    ...
    try {
  await fetch("https://e-connect-backend-2l74.onrender.com/thread", {
        method: "POST",
  ...
  const handleCreateGroup = async () => {
    ...
    try {
  const res = await fetch("https://e-connect-backend-2l74.onrender.com/create_group", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        setSelectedUsers([]);
        // Refresh groups
  const groupRes = await fetch(`https://e-connect-backend-2l74.onrender.com/get_user_groups/${userid}`);
        if (groupRes.ok) {
